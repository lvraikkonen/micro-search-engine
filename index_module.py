...
class IndexModule:
        ...
        for item in seg_list:
            ...
            if item !='' and not self.is_number(item) and item not in self.stop_words:
                ...
                if item in cleaned_dict:
                    ...
                    cleaned_dict[item] = 1
        return count, cleaned_dict

    # 将posting列表写入数据库
    def dump_postings_to_db(self, db_path):
    ...
    def construct_postings_lists(self):
        ...
        AVG_L = 0
        for filename in files: # 对每篇文章
            root = ET.parse(config['DEFAULT']['doc_dir_path'] + filename).getroot()
        ...
        self.dump_postings_to_db(config['DEFAULT']['db_path'])
